...
class Codebase:
    ...
    def emit_gdmodule_config_py(self, path: str, project_name: str):
        out = "# Generated by GdUnsharp, do not modify!\n\n"
        out += "def can_build(env, platform):\n"
        out += "    return True\n"
        out += "\n"
        out += "def configure(env):\n"
        out += "    pass\n"

        with open(f"{path}/config.py", "w") as out_file:
            out_file.write(out)

    def emit_gdmodule_scsub(self, path: str, project_name: str):
        out = "# Generated by GdUnsharp, do not modify!\n\n"
        out += 'Import("env")\n\n'
        out += "module_env = env.Clone()\n"
        out += 'module_env.add_source_files(env.modules_sources, "*.cpp")\n'
        out += 'module_env.Append(CPPPATH=["."])\n'

        with open(f"{path}/SCsub", "w") as out_file:
            out_file.write(out)

    def emit_gdmodule_register_types_h(self, path: str, project_name: str):
        out = "// Generated by GdUnsharp, do not modify!\n\n"
        out += "#pragma once\n\n"
        out += '#include "modules/register_module_types.h"\n\n'

        out += f"void initialize_{project_name}_module(ModuleInitializationLevel p_level);\n"
        out += f"void uninitialize_{project_name}_module(ModuleInitializationLevel p_level);\n"

        with open(f"{path}/register_types.h", "w") as out_file:
            out_file.write(out)

    def emit_gdmodule_register_types_cpp(self, path: str, project_name: str):
        out = "// Generated by GdUnsharp, do not modify!\n"
        out += '#include "register_types.h"\n'
        out += '#include "core/object/class_db.h"\n'
        out += "\n"

        godotic_classes = [
            t
            for t in self.get_all_namespace_types()
            if isinstance(t, CodeClass) and t.is_godotic and not t.is_external_type
        ]
        ...
        out += f"void initialize_{project_name}_module(ModuleInitializationLevel p_level) {{\n"
        out += "\tif (p_level != MODULE_INITIALIZATION_LEVEL_SCENE) {\n"
        out += "\t\treturn;\n"
        out += f"\t}}\n"
        out += "\n"
        ...
        for godotic_class in godotic_classes:
            if godotic_class.name == "RandomProjectileSpawner" or True:
                ...
                out += f"\tClassDB::register_class<{namespace_path}::{godotic_class.name}>();\n"
        out += "\n"
        ...
        with open(f"{path}/register_types.cpp", "w") as out_file:
            out_file.write(out)

    def emit_cpp(self, path: str, project_name: str):
        self.global_namespace.emit_cpp(path)
        self.emit_gdmodule_register_types_cpp(path, project_name)
        self.emit_gdmodule_register_types_h(path, project_name)
        self.emit_gdmodule_scsub(path, project_name)
...
def get_or_create_class_from_node(
    node: Node, namespace: CodeNamespace, usings: list[str], codebase: Codebase
):
    ...
    if class_id not in namespace.types_by_id:
        ...
        ):  # TODO: check if parent class is godotic, watch out for out of order class iteration
    ...
    for child_node in parent_node.named_children:
        match child_node.grammar_name:
            ...
            case (
                ...
                get_or_create_class_from_node(child_node, namespace, usings, codebase)
...
def fix_godot_virtuals(codebase: Codebase):
    godotics = [
        t
        for t in codebase.get_all_namespace_types()
        if isinstance(t, CodeClass) and t.is_godotic
    ]
    gdsharp_method_names = ["_Ready", "_Process", "_PhysicsProcess"]
    for godotic in godotics:
        for method in godotic.methods_by_id.values():
            if method.id in gdsharp_method_names:
                ...
                while base_class:
                    if method.id in base_class.methods_by_id:
                        has_override_parent = True
                        break
...
def load_external_types(codebase: Codebase):
    ...
    def make_godot_class(
        ...
        cpp_class.custom_include_path = include_path
    ...
    def make_collection_replacement(
        ...
        godot_collection.custom_include_path = (
            f"core/templates/{camel_to_snake(godot_collection.name)}.h"
        )
    ...
    ns_godotcpp = codebase.global_namespace
    ...
    make_godot_class("Area3D", ns_godotsharp, ns_godotcpp, "scene/3d/physics/area_3d.h")
    make_godot_class("Button", ns_godotsharp, ns_godotcpp, "scene/gui/button.h")
    make_godot_class(
        "ButtonGroup", ns_godotsharp, ns_godotcpp, "scene/gui/base_button.h"
    )
    make_godot_class(
        "CharacterBody3D",
        ns_godotsharp,
    ...
    make_godot_class("CheckBox", ns_godotsharp, ns_godotcpp, "scene/gui/check_box.h")
    make_godot_class("Color", ns_godotsharp, ns_godotcpp, "core/math/color.h")
    make_godot_class("ColorRect", ns_godotsharp, ns_godotcpp, "scene/gui/color_rect.h")
    make_godot_class("Control", ns_godotsharp, ns_godotcpp, "scene/gui/control.h")
    make_godot_class(
        "GpuParticles3D", ns_godotsharp, ns_godotcpp, "scene/3d/gpu_particles_3d.h"
    )
    make_godot_class(
        "HBoxContainer", ns_godotsharp, ns_godotcpp, "scene/gui/box_container.h"
    )
    make_godot_class("Label", ns_godotsharp, ns_godotcpp, "scene/gui/label.h")
    make_godot_class("Marker3D", ns_godotsharp, ns_godotcpp, "scene/3d/marker_3d.h")
    make_godot_class(
        "MarginContainer", ns_godotsharp, ns_godotcpp, "scene/gui/margin_container.h"
    )
    make_godot_class(
        "MeshInstance3D", ns_godotsharp, ns_godotcpp, "scene/3d/mesh_instance_3d.h"
    )
    make_godot_class(
        "NavigationAgent3D",
        ns_godotsharp,
        ns_godotcpp,
        include_path="scene/3d/navigation/navigation_agent_3d.h",
    )
    make_godot_class("Node", ns_godotsharp, ns_godotcpp, "scene/main/node.h")
    make_godot_class("Node3D", ns_godotsharp, ns_godotcpp, "scene/3d/node_3d.h")
    make_godot_class(
        "PackedScene", ns_godotsharp, ns_godotcpp, "scene/resources/packed_scene.h"
    )
    make_godot_class(
        "PanelContainer", ns_godotsharp, ns_godotcpp, "scene/gui/panel_container.h"
    )
    make_godot_class(
        "ProgressBar", ns_godotsharp, ns_godotcpp, "scene/gui/progress_bar.h"
    )
    make_godot_class(
        "ShaderMaterial", ns_godotsharp, ns_godotcpp, "scene/resources/material.h"
    )
    make_godot_class(
        "StaticBody3D",
        ns_godotsharp,
        ns_godotcpp,
        include_path="scene/3d/physics/static_body_3d.h",
    )
    make_godot_class(
        "Texture2D", ns_godotsharp, ns_godotcpp, "scene/resources/texture.h"
    )
    make_godot_class(
        "TextureRect", ns_godotsharp, ns_godotcpp, "scene/gui/texture_rect.h"
    )
    make_godot_class("Timer", ns_godotsharp, ns_godotcpp, "scene/main/timer.h")
    make_godot_class(
        "VBoxContainer", ns_godotsharp, ns_godotcpp, "scene/gui/box_container.h"
    )
...
modules_directory = "C:/prg/_git/_godot/_godot_itself/modules"
...
codebase.emit_cpp(out_path, project_name)
